...
from app.models import (
    ImportBatch, ImportRecord, Product, Employee, Department, Supplier, Customer,
    Purchase, PurchaseItem, Sale, SaleItem, Category, InventoryTransaction, User
)

# Standard Canonical Schema Mappings for Dynamic Auto-Mapping
ENTITY_FIELD_SCHEMAS: Dict[str, Dict[str, List[str]]] = {
    "products": {
        "sku": ["sku", "product_sku", "product no", "product_number", "item_code", "stock_code"],
        "name": ["name", "product_name", "title", "description", "item_name"],
        "category_name": ["category", "category_name", "cat_name", "department"],
        "purchase_price": ["cost", "cost_price", "purchase_price", "buy_price", "unit_cost"],
        "selling_price": ["price", "selling_price", "sell_price", "unit_price", "retail_price"],
    ...
    "employees": {
        "employee_code": ["employee_id", "employee_no", "emp_code", "staff_id", "badge_no"],
        "first_name": ["first_name", "given_name", "fname", "employee_name", "full_name", "name"],
...
def validate_and_stage_import(
    ...
    headers, raw_rows = parse_csv_or_excel(file_bytes, filename)
    ...
    for idx, raw_row in enumerate(raw_rows, start=1):
        ...
        if entity_key == "products":
            ...
            reorder_str = normalized.get("reorder_level", "5")
    ...
    return {
        ...
        "errors": [
            ...
            for r in staged_records if r["validation_status"] == "REJECTED"
        ]
    }
    ...
    for rec in records:
        ...
        if entity_key == "products":
            ...
            if not existing:
                ...
                if not dept:
                    dept = Department(name=dept_name, department_code=f"DEP-{uuid.uuid4().hex[:4].upper()}")
                    db.add(dept)
                    db.flush()
...
def generate_entity_template(entity_type: str) -> Tuple[str, str]:
    ...
    if entity_key == "products":
        headers = ["SKU", "Name", "Category", "Cost Price", "Selling Price", "Reorder Level", "Unit", "Barcode"]
        rows = [
            ["PRD-1001", "Wireless Optical Mouse", "Electronics", "12.50", "24.99", "10", "Units", "600123456789"],
            ["PRD-1002", "Mechanical Keyboard RGB", "Electronics", "45.00", "89.99", "5", "Units", "600123456790"]
        ]
        filename = "products_template.csv"
    elif entity_key == "employees":
        headers = ["Employee No", "First Name", "Last Name", "Email", "Phone", "Position", "Department"]
        rows = [
            ["EMP-2026-001", "Alice", "Smith", "alice.smith@company.com", "+263 77 123 4567", "CASHIER", "Sales"],
            ["EMP-2026-002", "Bob", "Jones", "bob.jones@company.com", "+263 71 987 6543", "WAREHOUSE_ASSISTANT", "Logistics"]
        ]
        filename = "employees_template.csv"
    elif entity_key == "suppliers":
        headers = ["Supplier Name", "Contact Person", "Email", "Phone", "Address"]
        rows = [
            ["Global Tech Distributors", "Jane Doe", "orders@globaltech.com", "+263 24 2123456", "12 Industrial Rd, Harare"],
            ["Apex Wholesalers", "Sam Wilson", "sales@apex.co.zw", "+263 29 2887766", "45 Main St, Bulawayo"]
        ]
...
def export_entity_data(db: Session, entity_type: str) -> Tuple[str, str]:
    ...
    if entity_key == "products":
        ...
        writer.writerow(["ID", "SKU", "Product Code", "Name", "Category ID", "Purchase Price", "Selling Price", "Stock Quantity", "Reorder Level", "Unit", "Barcode"])
        for p in products:
            writer.writerow([p.id, p.sku, p.product_code, p.name, p.category_id, p.purchase_price, p.selling_price, p.stock_quantity, p.reorder_level, p.unit, p.barcode])
        filename = f"export_products_{timestamp_str}.csv"

    elif entity_key == "employees":
        employees = db.query(Employee).all()
        writer.writerow(["ID", "Employee Code", "First Name", "Last Name", "Email", "Phone", "Position", "Status", "Created At"])
        for e in employees:
            writer.writerow([e.id, e.employee_code, e.first_name, e.last_name, e.email, e.phone, e.position, e.status, e.created_at])
        ...
        products = db.query(Product).filter(Product.active == True).all()
        writer.writerow(["SKU", "Name", "Stock Qty", "Unit Cost", "Unit Price", "Total Cost Value", "Total Retail Value"])
        for p in products:
            c_val = p.stock_quantity * p.purchase_price
            r_val = p.stock_quantity * p.selling_price
            writer.writerow([p.sku, p.name, p.stock_quantity, p.purchase_price, p.selling_price, f"{c_val:.2f}", f"{r_val:.2f}"])
...
def generate_import_template_csv(entity_type: str) -> str:
    content, _ = generate_import_template(entity_type)
    return content


def approve_import_batch(db: Session, batch_id: str, approver_user_id: Optional[int] = None) -> ImportBatch:
