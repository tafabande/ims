...
from app.models import Cart, CartItem, StockReservation, StorePickupOrder, Product, Sale, SaleItem, InventoryTransaction, Customer
...
def expire_stale_reservations(db: Session):
    ...
    """
    now = datetime.utcnow()
    stale_reservations = db.query(StockReservation).filter(
...
def checkout_cart_reservation(
...
) -> tuple[Sale, Optional[StorePickupOrder]]:
    """
    ...
    if cart.user_id and current_user_id and cart.user_id != current_user_id:
        raise HTTPException(status_code=403, detail="IDOR Protection: Unauthorized access to cart reservation.")

    if cart.status != "ACTIVE" or cart.expires_at <= datetime.utcnow():
        raise HTTPException(status_code=400, detail="Cart reservation has expired or is no longer active.")
...
def cancel_cart_reservation(db: Session, cart_id: int, current_user_id: Optional[int]):
    """
