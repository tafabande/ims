...
from app.models import Register, Shift
...
def open_shift(db: Session, open_data) -> Shift:
    ...
    if register.status == "OPEN":
        raise HTTPException(
            status_code=400,
            detail=f"Register '{register.register_code}' is currently OPEN by another shift",
        )

    # Verify active shift for employee
    active_shift = db.query(Shift).filter(Shift.employee_id == open_data.employee_id, Shift.status == "OPEN").first()
    if active_shift:
...
def get_active_shift_for_employee(db: Session, employee_id: int) -> Shift:
    shift = db.query(Shift).filter(Shift.employee_id == employee_id, Shift.status == "OPEN").first()
    return shift
