...
from app.models import Store, Warehouse, StoreStock, Product

def create_store(db: Session, store_data) -> Store:
    existing = db.query(Store).filter(Store.store_code == store_data.store_code).first()
    if existing:
        raise HTTPException(status_code=400, detail=f"Store code '{store_data.store_code}' already exists")
...
def create_warehouse(db: Session, wh_data) -> Warehouse:
    ...
    if existing:
        raise HTTPException(status_code=400, detail=f"Warehouse code '{wh_data.warehouse_code}' already exists")
