...
def list_operational_cases(
    ...
    """
    return case_service.list_cases(status_filter=status_filter, type_filter=type_filter, db=db)


@router.get("/{case_number}")
def get_case_details_and_timeline(
    case_number: str,
    auth_ctx: dict = Depends(require_permission("inventory:view")),
    db: Session = Depends(get_db),
):
    ...
    """
    c = case_service.get_case_by_number(case_number, db=db)
    if not c:
...
def create_operational_case(
    ...
    """
    return case_service.create_case(payload, db=db)
...
def execute_case_decision(
    ...
    res = case_service.execute_decision(case_number, decision, reviewer, comment, db=db)
