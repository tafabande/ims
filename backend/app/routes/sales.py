...
def get_sales(skip: int = 0, limit: int = 100, db: Session = Depends(get_db), auth: dict = Depends(require_permission("sales:read"))):
    sales = db.query(Sale).order_by(Sale.created_at.desc()).offset(skip).limit(limit).all()
    return sales

@router.post("/", response_model=SaleResponse, status_code=status.HTTP_201_CREATED)
def create_sale(sale_in: SaleCreate, db: Session = Depends(get_db), auth: dict = Depends(require_permission("sales:create"))):
