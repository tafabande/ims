    ...
    "payment_accounts": [
        {
            "id": "ACC-001",
            "code": "CASH-STORE-A",
            "name": "Main Cash Till Drawer - Store A",
            "type": "CASH_DRAWER",
            "status": "ACTIVE",
        },
        {
            "id": "ACC-002",
            "code": "BANK-CBZ-01",
            "name": "CBZ Corporate Operations Account",
            "type": "BANK_ACCOUNT",
            "status": "ACTIVE",
        },
    ],
...
def get_enterprise_registry(registry_name: str, auth_ctx: dict = Depends(require_permission("inventory:view"))):
    """
...
def request_registry_mutation(
    ...
    if not reason:
        raise HTTPException(
            status_code=400,
            detail="Justification reason required for registry mutation.",
        )
