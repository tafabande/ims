...
def reject_pending_request(
    ...
    return approval_service.reject_request(
        db, request_id, approver_id, review_data.rejection_reason or "Request rejected."
    )
...
def request_jit_privilege_elevation(
    ...
    if not reason:
        raise HTTPException(
            status_code=400,
            detail="Reason justification is required for privilege elevation.",
        )
    ...
    new_grant = {
        ...
        "expires_at": (datetime.now(UTC) + timedelta(minutes=expiry_minutes)).isoformat(),
    }
...
def approve_jit_privilege_elevation(
    ...
    for grant in MOCK_JIT_ELEVATIONS:
        if grant["grant_code"] == grant_code:
            ...
            grant["approved_at"] = datetime.now(UTC).isoformat()
            return {
