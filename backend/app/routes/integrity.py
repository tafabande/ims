...
from app.schemas import (
    InventoryAnomalyResponse, InvestigationCaseResponse,
    StockReservationRequest, StockReservationResponse,
    LineageExplanationResponse, BLELocationResponse
)
from app.services import integrity_service
from app.services.iam_service import require_permission
from app.models import InventoryAnomaly, InvestigationCase, Product, BLEDeviceLocation

router = APIRouter(prefix="/api/integrity", tags=["Inventory Integrity & Operational Intelligence Engine"])
...
def get_digital_twin_spatial_map(
    ...
    layout = {
        ...
        "zones": [
            {"code": "AISLE-A", "name": "Aisle A - High Velocity Laptops", "shelves": ["Shelf A1", "Shelf A2", "Shelf A3"]},
            {"code": "AISLE-B", "name": "Aisle B - Peripherals & Accessories", "shelves": ["Shelf B1", "Shelf B2", "Shelf B3"]},
            {"code": "RECEIVING-BAY", "name": "Receiving & Quality Quarantine", "shelves": ["Quarantine Bay 01", "Quarantine Bay 02"]}
        ],
