...
from app.schemas import (
    IntegrationAccountCreate, IntegrationAccountResponse,
    IntegrationApiKeyResponse, IntegrationActivityLogResponse,
    ExternalProductCreate, ExternalEmployeeCreate, ExternalCustomerCreate,
    ExternalSupplierCreate, ExternalSaleCreate, ExternalPurchaseCreate
)
from app.services import integration_service
from app.services.iam_service import require_permission
from app.models import IntegrationAccount, IntegrationApiKey, IntegrationActivityLog
...
@router.post("/accounts", response_model=IntegrationAccountResponse, status_code=status.HTTP_201_CREATED)
...
def external_ingest_product(
    ...
    try:
        ...
        integration_service.log_integration_activity(
            db, account.account_id, "/api/v1/integrations/products", "POST", 201, request.client.host if request.client else "127.0.0.1"
        )
        return {
            "status": "success",
        ...
        integration_service.log_integration_activity(
            db, account.account_id, "/api/v1/integrations/products", "POST", 400, request.client.host if request.client else "127.0.0.1", str(e)
        )
...
def external_ingest_employee(
    ...
    try:
        ...
        integration_service.log_integration_activity(
            db, account.account_id, "/api/v1/integrations/employees", "POST", 201, request.client.host if request.client else "127.0.0.1"
        )
        return {
            "status": "success",
        ...
        integration_service.log_integration_activity(
            db, account.account_id, "/api/v1/integrations/employees", "POST", 400, request.client.host if request.client else "127.0.0.1", str(e)
        )
        raise HTTPException(status_code=400, detail=str(e))
