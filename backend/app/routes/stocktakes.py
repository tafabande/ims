...
def create_stocktake_session(stk_data: StocktakeCreate, db: Session = Depends(get_db)):
    return stocktake_service.create_stocktake(db, stk_data)


@router.post("/{stocktake_id}/approve", response_model=StocktakeResponse)
def approve_stocktake_session(stocktake_id: int, approved_by_emp_id: int, db: Session = Depends(get_db)):
