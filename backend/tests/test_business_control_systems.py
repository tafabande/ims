...
from app.models import ApprovalRequest, Category, Product, User
...
def test_approval_engine_four_eyes_principle():
    ...
    for uid in [1, 2]:
        if not db.query(User).filter(User.id == uid).first():
            u = User(
                ...
                role="MANAGER" if uid == 2 else "STAFF",
    ...
    self_approve_res = client.post(f"/api/approvals/{req_id}/approve", headers={"X-User-Role": "MANAGER"})
    ...
    approved_res = client.post(f"/api/approvals/{req_id}/approve", headers={"X-User-Role": "MANAGER"})
...
def test_inventory_reconciliation_engine():
    ...
    """
    scan_res = client.post("/api/reconciliation/scan", json={}, headers={"X-User-Role": "MANAGER"})
...
def test_commercial_pricing_margin_protection():
    ...
    neg_mgr_res = client.post(
        "/api/pricing/check-negotiation",
        json={"product_id": prod_id, "offered_price": 95.0, "user_role": "MANAGER"},
        headers={"X-User-Role": "MANAGER"},
    )
