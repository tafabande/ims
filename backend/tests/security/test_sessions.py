...
def test_login_creates_active_session():
    ...
    # Verify session recorded in /auth/sessions with authenticated header
...
def test_revoke_session_invalidates_session():
    ...
    revoke_res = client.post(f"/auth/revoke-session/{target_id}", headers=headers)
...
def test_unauthenticated_session_access_rejected():
    """
    MA-05 Security Test:
    Unauthenticated requests to /auth/sessions and /auth/revoke-session must return HTTP 401 Unauthorized.
    """
    res_list = client.get("/auth/sessions")
    assert res_list.status_code == 401

    res_revoke = client.post("/auth/revoke-session/any-session-id")
...
def test_non_admin_cannot_revoke_other_user_session():
    ...
    admin_res = client.post("/auth/login", json={"username": "admin", "password": "adminpassword"})
    admin_token = admin_res.json()["access_token"]
    admin_sessions = client.get("/auth/sessions", headers={"Authorization": f"Bearer {admin_token}"}).json()
    admin_session_id = admin_sessions[0]["session_id"]

    # 2. Staff logs in
    staff_res = client.post("/auth/login", json={"username": "staff", "password": "staffpassword"})
    staff_token = staff_res.json()["access_token"]

    # 3. Staff attempts to revoke Admin's session -> Rejected 404 (or 403)
    revoke_res = client.post(
        f"/auth/revoke-session/{admin_session_id}",
...
def test_invalid_login_credentials_rejected():
    ...
    response_fake = client.post("/auth/login", json={"username": "unseeded_admin", "password": "admin123"})
    assert response_fake.status_code == 401

    # Inactive/Deactivated account cannot log in
    response_deact = client.post("/auth/login", json={"username": "deactivated_user", "password": "password123"})
