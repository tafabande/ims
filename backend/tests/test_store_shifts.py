...
from app.models import Store, Register, Employee
...
def test_shift_open_close_cash_variance():
    ...
    emp = Employee(employee_code=emp_code, first_name="Sam", last_name="Cashier", email=f"sam_{uuid.uuid4().hex[:4]}@ims.local")
    db.add(emp)
    db.commit()
    ...
    close_res = client.post(f"/api/shifts/{shift_id}/close", json={
        "actual_cash": 195.0,
