...
from app.models import Product, Employee, ImportBatch, ImportRecord, IntegrationAccount, IntegrationApiKey
...
def test_csv_staging_and_sha256_duplicate_detection():
    ...
    batch2, records2, is_dup2, dup_msg2 = ingestion_service.stage_and_validate_import(
        db=db,
...
def test_dynamic_column_mapping_and_validation():
    ...
    batch, records, is_dup, _ = ingestion_service.stage_and_validate_import(
        db=db,
    ...
    rejected_rec = [r for r in records if r.validation_status == "REJECTED"][0]
    assert "cannot be negative" in rejected_rec.error_message.lower() or "selling price" in rejected_rec.error_message.lower()
...
def test_manager_approval_workflow_promotes_staging_to_core_db():
    ...
    batch, records, _, _ = ingestion_service.stage_and_validate_import(
        db=db,
...
def test_integration_accounts_and_api_key_authentication():
    ...
    # 2. Issue API Key
    key_rec, plain_text_key = integration_service.generate_api_key_for_account(
        db=db,
    ...
    auth_res = integration_service.verify_integration_api_key(
        ...
        required_scope="products:create"
    )
    ...
    with pytest.raises(Exception) as exc_info:
        integration_service.verify_integration_api_key(
            db=db,
            api_key=plain_text_key,
            required_scope="employees:delete"
        )
